...
static void initialize_resources(void)
{
	...
	size = sizeof(network_bearers[0]);
	lwm2m_engine_set_res_buf("4/0/1/0", &network_bearers[0], size, size,
				  LWM2M_RES_DATA_FLAG_RO);

	lwm2m_engine_create_res_inst("4/0/1/1");
	lwm2m_engine_set_res_buf("4/0/1/1", &network_bearers[1], size, size,
				  LWM2M_RES_DATA_FLAG_RO);
	...
	size = attr_get_size(ATTR_ID_ipv6_addr);
	lwm2m_engine_set_res_buf("4/0/4/1", str, size, size, LWM2M_RES_DATA_FLAG_RO);
#endif

	lwm2m_engine_create_res_inst("4/0/7/0");
	str = (char *)attr_get_quasi_static(ATTR_ID_lte_apn);
	size = attr_get_size(ATTR_ID_lte_apn);
	lwm2m_engine_set_res_buf("4/0/7/0", str, size, size, LWM2M_RES_DATA_FLAG_RO);
